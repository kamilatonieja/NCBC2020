...
void Xoodoo::calculateRoundConstants()
{
	...
	for (unsigned int i = 0; i < 7; i++)
	{
        std::cout << "p"<<i<<"="<< p << " ";
		rc_p.push_back(p);
		p = p ^ (p << 2); // przesuniecie bitowe << , ^ xor // 1)0b1 xor 0b100 = 0b101; 2) 0b100 xor 0b10000 = 0b10100 3) 0b10100 xor 0b1010000 =
		if ((p & 16) != 0) p ^= 22; // & and 16=0b10000        1)0b100 xor 0b10110 = 0b10010
		if ((p &  8) != 0) p ^= 11; // 8= 0b1000
	}
    std::cout << "" << std::endl;
}
...
void Xoodoo::stepIota(XoodooState &A, int i) const
{
    ...
    std::cout <<std::hex << "stepIota:"<<rc << std::endl;
	A[0][0] = A[0][0] ^ rc;
